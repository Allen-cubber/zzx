...
真实世界的机器人任务往往不是单步操作，而是需要跨越多个子技能阶段的长程序列——例如"打开抽屉 → 放入物体 → 关上抽屉"。如何让 VLA 模型可靠地完成这类任务，是目前研究的核心难点之一。主流方向分为两类：

- **分层规划 + 执行分离**：用高层 VLM 负责任务分解，低层策略执行每个子任务。缺陷是两者缺乏共享的潜在空间，容易产生决策不一致和系统延迟导致的过时指令。
- **端到端统一规划与执行**：将任务规划和动作生成整合进同一个模型，消除模块间的信息割裂，是目前更受关注的方向。

### 4.1 AtomicVLA

**论文**：[AtomicVLA: Unlocking the Potential of Atomic Skill Learning in Robots](https://arxiv.org/abs/2603.07648)（arXiv:2603.07648，2026 年 3 月）

**核心思路**：将复杂任务分解为有语义含义的**原子技能序列**（pick、place、turn、open、close 等），并为每种原子技能配备一个专属的动作专家，通过技能引导的混合专家架构（SG-MoE）在统一框架内完成规划与执行。详细解析见 [[academic/AtomicVLA/index.md]]。

**三个关键设计**：

1. **Think-Act 自适应切换**：引入 `[think]` 和 `[act]` 两个特殊 token，模型自主判断当前步骤应输出任务计划还是执行动作。思考模式只在任务启动或子技能切换时触发，大幅降低规划开销的同时保持了执行的连贯性。

2. **SG-MoE（技能引导混合专家）**：每种原子技能通过类似扩散模型噪声调度的方式嵌入成一个高维向量，路由器据此动态激活对应的技能专家。最终动作由共享专家（保留 π0 通用能力）与被激活的技能专家加权合成，兼顾泛化与精度。

3. **持续学习无遗忘**：新技能来临时，只新增一个专家模块并扩展路由分支，其余专家权重完全冻结。实验中 π0.5 学完新技能后旧技能平均下降 15%，AtomicVLA* 只下降 1.3%。

**实验结果**：
- LIBERO-LONG：95.2%（vs π0 的 85.2%，提升 **+10%**）
- CALVIN 平均任务长度：4.27（vs π0.5 的 4.02，提升 **+0.25**）
- 真实 Franka 机械臂长程任务：63.3%（vs π0.5 的 45.0%，提升 **+18.3%**）
- 消融：SG-MoE 相比标准 token 级 MoE 在 LIBERO-LONG 上高出 **+6.6%**，充分说明以原子技能为路由信号而非 token 级路由是关键。