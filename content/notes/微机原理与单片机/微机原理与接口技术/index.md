...
**5. 标志寄存器 (1个16位) ⭐ (重点掌握)**
![[Pastedimage20260321115443.png]]
*   **FLAGS:** 包含9个有意义的标志位。
    *   **状态标志 (6个):** 反映ALU运算结果的特征。
        *   **CF (Carry Flag):** 进位标志。**无符号**数运算最高位产生进/借位时CF=1。`STC`置1, `CLC`清0, `CMC`取反。
        *   **PF (Parity Flag):** 奇偶标志。结果低8位中'1'的个数为偶数时PF=1。
        *   **AF (Auxiliary Carry Flag):** 辅助进位标志。字节运算时，D3->D4有进/借位时AF=1。用于**BCD运算**调整 (`DAA`, `DAS`)。
        *   **ZF (Zero Flag):** 零标志。运算结果为0时ZF=1。
        *   **SF (Sign Flag):** 符号标志。运算结果最高位为1时SF=1 (表示负数，对有符号数)。
        *   **OF (Overflow Flag):** 溢出标志。**有符号**数运算结果超出表示范围时OF=1。
            *   字节: -128 ~ +127
            *   字: -32768 ~ +32767
...
![[Pasted image 20260321115501.png]]
...
*   **例 3.13**: `MOV AX, [BX][SI]`
    *   📝 设 `DS=3000H`, `BX=1200H`, `SI=0500H`, 内存 `(31700H)= CD H`, `(31701H)= AB H`。AX16 -> AH8 AL8
...
*   **7) `DAS` 减法的十进制调整指令 (Decimal Adjust for Subtraction)**
    *   格式: `DAS`
    *   功能: 对`AL`中两个压缩BCD（4位2进制数表示一位十进制数）数减法后的结果进行调整。
    *   用法: 通常在`SUB`或`SBB`之后使用。
    *   操作:
        1.  如果`AL`低4位 > 9 或 `AF`=1, 则 `AL` ← `AL` - 06H, `AF`=1。
        2.  如果`AL`高4位 > 9 或 `CF`=1, 则 `AL` ← `AL` - 60H, `CF`=1。
...
1.  **采用I/O接口的必要性** 🤔
    ...
    *   **对比**：
        *   ✅ CPU与存储器：速度在同一数量级，存储器有数据缓冲能力（buff），数据交换方便。
...
*   **主要组成部分**:
    *   **数据总线缓冲器**: 8位双向三态（0，1，Z）缓冲器，连接CPU数据总线 ($D_0 \sim D_7$)。
...
*   **[例1] 开关与LED控制 (P12-13)**
    *   **硬件**: 8255A (280H-283H)，端口B & C下部接开关（输入），端口A & C上部接LED（输出）。
    *   **要求**: 方式0，读开关状态，对应点亮LED。
...
3.  **中断向量表** 🗺️
    ...
    *   **中断向量表示例**:
        ...
        *   **[例8.2]** 在中断向量表中，地址 $0040H$ 处内容为 $240BH$，地址 $0042H$ 处内容为 $D169H$。
            *   向量起始地址为 $0040H$。
            *   对应的中断类型号 $n = 40H / 4 = 10H = 16D$。(计算: $40H = 01000000B$，右移两位 $00010000B = 10H$)
...
*   描述时序的四个基本时间单位：
	* 振荡周期：即时钟脉冲的周期，其值为时钟频率fosc的倒数。
	* 状态周期：每个状态周期由2个振荡周期组成，用P1、P2表示。
	* 机器周期：包含6个状态周期，用S1~S6表示，是单片机最小的时间单位。时钟频率12MHz->机器周期1微秒。机器周期12/fosc。
...
*   **计数脉冲来源**：
    ...
    2.  **作定时器时 (C/T#=0)**：
        *   计数脉冲由<font color="orange">系统时钟</font>经过**12分频**后提供（即机器周期脉冲）。
...
|  1  |  0  |  2  | 8位**自动重装初值**的定时器/计数器 (TLx计数，THx存放自动重装的初值)                                                                                                      |
...
*   格式：
    ...
    *   高4位与定时器/计数器相关：
        *   <font color="orange">TF1</font> (Timer 1 Overflow Flag)：T1溢出中断标志。T1计数溢出时，由硬件自动置1。CPU响应中断后，若采用中断方式，硬件**不会**自动清零TF1，<font color="orange">需在中断服务程序中用软件清零</font>。
...
4.  **波特率和发送/接收时钟**
    *   (1) <font color="orange">波特率</font> (Baud Rate) bit rate
...
**【例5-1】** 设8051的fosc = 12MHz，串行口工作方式1，串行通信波特率为4800bps，T1设为方式2，求T1初值并编写串行口初始化程序。取SMOD = 1。
...
*   定时器T1和串行口的初始化程序如下:
    ```assembly
    MOV TMOD, #20H   ; T1设为定时方式2 (8位自动重装) 0010 0000
    MOV TL1,  #0F3H   ; 置定时器初值
