...
本章的核心是探讨**数字信号如何在不经过调制的情况下，直接在信道上传输**。整个章节围绕着两个核心挑战展开：<font color="orange">带宽受限</font>和<font color="orange">信道噪声</font>，并层层递进地给出了理论和工程上的解决方案。

**本章的学习可以看作是一条解决问题的路径：**

1.  **起点：什么是基带传输？** 🛣️
    *   首先定义什么是**数字基带信号**（未经调制的电脉冲）和**基带传输系统**（发送、信道、接收）。并介绍了各种基本的信号**波形**（线路码型），如单/双极性码、归零/不归零码、AMI码、HDB3码、曼彻斯特码等。这是后续所有讨论的基础。

2.  **第一个挑战：带宽限制与码间串扰 (ISI)** 💥
    *   现实世界的信道都是<font color="orange">带限</font>的，这会导致脉冲波形在时域上展宽、产生“拖尾”。当码元速率过高时，前一个码元的“拖尾”会干扰到后一个码元的抽样判决，这就是**码间串扰 (ISI)**。
    *   **理论解决方案**：为了在抽样时刻彻底消除ISI，提出了**奈奎斯特第一准则**。它给出了无码间串扰的充要条件。
    *   **理想模型**：“最窄带”系统（理想低通），频带利用率最高（2 Baud/Hz），但其滤波器物理上不可实现。
    *   **工程解决方案**：**升余弦滚降系统**。通过牺牲一部分带宽（引入滚降系数 $\alpha$），换取物理上可实现的、特性平滑的滤波器。

3.  **另辟蹊径：如何“利用”码间串扰？** 🤔
    *   奈奎斯特准则的目标是“消灭”ISI。但我们能否反其道而行之，主动引入一种**可控的、已知的**ISI，并利用它来达到奈奎斯特带宽，同时又使用物理可实现的滤波器？
    *   **解决方案**：**部分响应系统**（如I型：双二进制系统）。它允许在抽样点上存在来自前一个码元的确定性干扰。
    *   **新问题**：这种方法在解码时需要参考前一个码元的判决结果，一旦发生错误，就会像多米诺骨牌一样连续出错，即**误码扩散**。
    *   **解决新问题**：**预编码**技术。在发送端预先进行模2加运算，巧妙地将解码时对“前一符号”的依赖关系消除，从而根治了误码扩散。

4.  **第二个挑战：信道噪声与最佳接收** 🔊
    *   到目前为止，我们只考虑了码间串扰。但现实中最大的敌人是**加性高斯白噪声 (AWGN)**。如何在噪声的干扰下，做出最准确的判决？
    *   **理论基础**：**统计判决理论**。通过似然函数、最大后验概率 (MAP) 和最大似然 (ML) 准则，推导出误码率最小意义上的**最佳判决**方法。
    *   **核心器件**：实现最佳判决的接收机前端是**匹配滤波器**。它的唯一使命，就是在抽样时刻，最大化输出信号的**信噪比**。
    *   **性能衡量**：最终的系统误码率 $P_e$ 可以用 **Q函数** 来精确计算。

5.  **性能评估与波形选择** 📊
    *   利用推导出的误码率公式，我们可以定量比较不同信号波形的优劣。最重要的结论是：**反极性信号**（如BPSK基带）比**正交信号**（如OOK）具有**3dB**的性能优势。
    *   **实验工具**：**眼图**。通过示波器观测接收信号波形的叠加，可以直观地评估系统的码间串扰、噪声容限、定时抖动等综合性能。一个“张开的大眼睛”代表一个健康的通信系统。

**总结**：本章从定义基带信号开始，围绕**码间串扰**和**噪声**两大核心问题，分别提出了奈奎斯特准则/部分响应系统和匹配滤波器/最佳接收理论，最终形成了完整的数字基带传输理论体系，并通过眼图提供了工程实践的评估方法。
...
-   **不归零码 (NRZ)**：
    -   单边主瓣宽度为 $1/T = R_b$ (码元速率)。
    -   在 $f=0$ 处有较大的值（直流分量）。
-   **归零码 (RZ)**：
    -   单边主瓣宽度为 $2/T = 2R_b$，是NRZ码的两倍。
...
-   **频域条件**：
    ...
    $$ \boxed{ \sum_{i=-\infty}^{\infty} H(f - \frac{i}{T}) = C \quad (\text{常数})\ \ \ \ \ } $$
...
由于<font color="orange">加性高斯白噪声</font>经过线性滤波器（匹配滤波器、均衡器等）后，其高斯特性保持不变（输出通常是**有色**高斯噪声，而不是**白**噪声）。因此，抽样值 $n_0$ 依然是一个<font color="orange">高斯随机变量</font>，其概率密度函数为：
$$
...
$$\frac{d P_E(\gamma_0)}{d \gamma_0} = 0$$

利用莱布尼茨积分求导法则（变上限积分求导为正，变下限积分求导为负）：

- 第一项 $\int_{-\infty}^{\gamma_0} p(z|s_1)dz$ 对 $\gamma_0$ 求导得到 $p(\gamma_0|s_1)$
    
- 第二项 $\int_{\gamma_0}^{\infty} p(z|s_2)dz$ 对 $\gamma_0$ 求导得到 $-p(\gamma_0|s_2)$
    

代入导数方程：

$$P(s_1)p(\gamma_0|s_1) - P(s_2)p(\gamma_0|s_2) = 0$$

移项整理得到**最大似然比方程**：

$$\frac{p(\gamma_0|s_1)}{p(\gamma_0|s_2)} = \frac{P(s_2)}{P(s_1)}$$

代入已知的 $p(z|s_1)$ 和 $p(z|s_2)$ 公式代入上述方程。注意此时 $z$ 取值为最佳门限 $\gamma_0^*$：

$$\frac{\frac{1}{\sqrt{2\pi}\sigma_0} \exp\left(-\frac{(\gamma_0^*-a_1)^2}{2\sigma_0^2}\right)}{\frac{1}{\sqrt{2\pi}\sigma_0} \exp\left(-\frac{(\gamma_0^*-a_2)^2}{2\sigma_0^2}\right)} = \frac{P(s_2)}{P(s_1)}$$

两边同时取自然对数 $\ln$，消去指数：
$$\frac{(\gamma_0^*-a_2)^2 - (\gamma_0^*-a_1)^2}{2\sigma_0^2} = \ln\left(\frac{P(s_2)}{P(s_1)}\right)$$
**最终公式：**

$$\gamma_0^* = \frac{a_1+a_2}{2} + \frac{\sigma_0^2}{a_1 - a_2} \ln\left(\frac{P(s_2)}{P(s_1)}\right)$$
...
$$P_e = P_1 \cdot P(e|s_1) + P_2 \cdot P(e|s_2)$$
$P_e$一般形式的表达式为：

$$P_e = P_1 Q\left( \frac{a_1 - V_{th}}{\sigma_n} \right) + P_2 Q\left( \frac{V_{th} - a_2}{\sigma_n} \right)$$
...
$$
\text{最佳接收} \implies \text{最大化 } d\_{max}^2=\frac{(a_1-a_2)^2}{\sigma_0^2}
$$
...
$$
频域表达式为：$$H(f) = k \cdot[S_1^*(f) - S_1^*(f)] e^{-j2\pi f T_0}$$
...
$$d_{\max }^{2} = \frac{1}{2 N_{0}} \int_{0}^{T}[s_{1}(T-\tau)-s_{2}(T-\tau)]^2d\tau$$
展开平方项，得到三部分积分：

$$d_{\max }^{2} = \frac{1}{2 N_{0}}\left(\underbrace{\int_{0}^{T} s_{1}^{2}(T-\tau) d \tau}_{\text{能量项 } 1} - 2 \underbrace{\int_{0}^{T} s_{1}(T-\tau) s_{2}(T-\tau) d \tau}_{\text{互相关项}} + \underbrace{\int_{0}^{T} s_{2}^{2}(T-\tau) d \tau}_{\text{能量项 } 2}\right)$$
- 信号 $s_1(t)$ 的能量 $E_{s1}$：
    $$\int_{0}^{T} s_{1}^{2}(T-\tau) d \tau = \int_{0}^{T} s_{1}^{2}(t) d t = E_{s_{1}}$$
- 信号 $s_2(t)$ 的能量 $E_{s2}$：
    $$\int_{0}^{T} s_{2}^{2}(T-\tau) d \tau = \int_{0}^{T} s_{2}^{2}(t) d t = E_{s_{2}}$$
- 互相关项：
    $$\int_{0}^{T} s_{1}(T-\tau) s_{2}(T-\tau) d \tau = \int_{0}^{T} s_{1}(t) s_{2}(t) d t$$

定义 $s_1(t)$ 与 $s_2(t)$ 之间的**归一化互相关系数**$\rho$：
$$\rho = \frac{1}{\sqrt{E_{s_{1}} E_{s_{2}}}} \int_{0}^{T} s_{1}(t) s_{2}(t) d t$$

- 注：$-1 \le \rho \le 1$
将上述能量项和相关系数代回 $d_{max}^2$ 的表达式（式 5.7.51）：

$$d_{\max }^{2} = \frac{E_{s_{1}}+E_{s_{2}}-2 \rho \sqrt{E_{s_{1}} E_{s_{2}}}}{2 N_{0}}$$

为了得到通用的误码率公式，通常引入以下假设：

1. 定义平均比特能量 $E_b$：
    $$E_{b} = P(s_1)E_{s_1} + P(s_2)E_{s_2}$$
2. 假设先验等概（$P(s_1) = P(s_2) = 1/2$）：
    $$E_{b} = \frac{1}{2}(E_{s_1} + E_{s_2})$$
3. 假设信号能量相等（$E_{s_1} = E_{s_2}$）：
    此时可得：
    $$E_{b} = E_{s_1} = E_{s_2}$$
在此条件下，$d_{max}^2$ 极大地简化为：

$$d_{\max }^{2} = \frac{2 E_{b} - 2 \rho E_{b}}{2 N_{0}} = \frac{E_{b}(1-\rho)}{N_{0}}$$

最佳接收机的误码率由 $Q(d_{max})$ 决定：

$$\boxed{ P_{e} = Q\left( \sqrt{\frac{E_{b}(1-\rho)}{N_{0}}} \right) }$$
#### 4.2 两种重要信号类型
...
*   **时域条件**：系统总冲激响应 $h(t)$ 在抽样时刻 $kT$ 的值应满足：
    $$
    h(kT) =
    \begin{cases}
    1, & k=0 \\
    ...
    \end{cases}
    $$
    > 含义：在对当前码元抽样时，所有其他码元的响应值都恰好为零。

*   **频域条件**：系统总频率响应 $H(\omega)$ 经过周期性延拓和叠加后，在一个周期内应为一个常数。
    $$
    \sum_{i=-\infty}^{\infty} H\left(\omega + \frac{2\pi i}{T}\right) = \text{常数}, \quad |\omega| \le \frac{\pi}{T}
    $$
    > 含义：这是设计无码间串扰滤波器的指导原则。
...
*   **预编码规则**（消除误码扩散）：
    $$
    \boxed{d_k = a_k \oplus d_{k-1}}
    $$
    > 含义：$a_k$是原始信息比特，$d_k$是预编码后的比特。通过这个简单的异或操作，接收端可以实现无记忆判决。
...
*   **二进制系统通用误码率**：
    ...
    \boxed{P_e = Q\left(\frac{a_1 - a_2}{2\sigma_0}\right)}
    $$
    > 含义：误码率取决于两个理想信号抽样值 $a_1, a_2$ 的**距离**与**噪声强度** $\sigma_0$ 之比。这是本章最重要的公式之一。

*   **匹配滤波器冲激响应**：
    $$
    \boxed{h(t) = k \cdot [s_1(T-t) - s_2(T-t)]}
    $$
    > 含义：最佳接收滤波器的形状是**差值信号**的**时间反转**模板。它能使抽样时刻的输出信噪比最大化。
...
*   **通用误码率公式 (含相关系数 $\rho$)**：
    ...
    $$
    > 含义：将信号能量 $E_b$、噪声功率谱密度 $N_0$ 和信号波形相似度 $\rho$ 联系在一起的最终性能公式。

#### 4. 两种典型信号的误码率

*   **反极性信号 (Antipodal)**，如BPSK基带信号 ($\rho = -1$)：
    $$
    P_E = Q\left(\sqrt{\frac{2E_b}{N_0}}\right)
    $$

*   **正交信号 (Orthogonal)**，如OOK或FSK ($\rho = 0$)：
    $$
    P_E = Q\left(\sqrt{\frac{E_b}{N_0}}\right)
    $$
    > 含义：明确了反极性信号的抗噪声性能优于正交信号。
