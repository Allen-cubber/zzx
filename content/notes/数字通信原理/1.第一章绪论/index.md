...
#### 🧱 数字通信系统详细组成模型
![](content/notes/数字通信原理/1.第一章绪论/Pastedimage20250906173427.png)
-   **信源**: 产生待传输的消息。
-   **信源编码器**: 完成模拟信号到数字信号的转化，并进行数据压缩以提高信息传输速率。
-   **信道编码器**: 对数字信号进行差错控制编码，以抵抗信道噪声的干扰，提高信号传输质量。
...
衡量单位时间内系统能传输信息量的大小。
-   **波特率 (Baud Rate, $R_S$)**: 单位时间内传输的<font color="orange">码元</font>数，也称**符号速率**。单位是**波特 (Baud)**。
...
衡量接收信号的准确程度。
-   **信噪比 (SNR)**: 信号平均功率 $S$ 与噪声平均功率 $N$ 之比。
    $SNR = \frac{S}{N}$
    通常用分贝 (dB) 表示：
    $(SNR)_{dB} = 10\lg(\frac{S}{N})$

-   **误码率 ($P_s$) 与 误信率 ($P_b$)**
    -   **误码率$P_s$**: 错误码元符号数 / 总码元符号数
    -   **误比特率$P_b$**: 错误比特数 / 总比特数，是更常用的指标。
    -   总量的关系： 传输的总比特数 = 传输的总码元数 $\times \log_2 M$。
    -   错误的对应关系：当一个码元发生判决错误时，它通常被误判为距离它最近的那个相邻码元。**如果使用格雷码**，相邻的两个码元之间，**只有 1 个比特**是不同的。因此，错 1 个码元 $\approx$ 错 1 个比特。则有：
$$\text{误信率} \approx \frac{\text{误码率}}{\log_2 M}$$
#### 3. 频带利用率 ($\eta$)
