...
*   **抽样**：把<font color="orange">连续时间</font>模拟信号转换成<font color="orange">时间离散</font>、<font color="orange">连续幅度</font>的抽样信号。
*   **量化**：把<font color="orange">连续幅度</font>抽样信号转换成<font color="orange">时间离散</font>、<font color="orange">幅度离散</font>的数字信号。
*   **编码**：将量化后的数字信号映射成<font color="orange">二进制码组</font>。
...
| **比较维度** | **理想抽样**                                  | **自然抽样**                             | **平顶抽样**                                                     |
| -------- | ----------------------------------------- | ------------------------------------ | ------------------------------------------------------------ |
| **物理模型** | **冲激脉冲**相乘                                | **矩形脉冲**相乘                           | **采样+保持**                                                    |
| **时域波形** | 一串强度不同的冲激函数 $\delta(t)$。                  | 脉冲顶部随原信号 $x(t)$ 变化，保留波形形状。           | 脉冲顶部是**平**的，在持续时间内电平恒定。                                      |
| **数学运算** | $x_s(t) = x(t) \cdot \sum \delta(t-nT_s)$ | $x_s(t) = x(t) \cdot \sum p(t-nT_s)$ | $x_s(t) = [x(t)\sum \delta(t-nT_s)] * h(t)$<br>(先冲激抽样，再卷矩形窗) |
| **频域特征** | **频谱周期搬移**。幅度为常数 $1/T_s$。                 | **频谱周期搬移**。幅度受脉冲串的傅里叶系数 $C_n$ 加权。    | **频谱周期搬移 + 畸变**。搬移后的谱被 $Sa(f)$ 函数包络加权。                       |
| **孔径效应** | **无**                                     | **无** (频谱形状未变，仅幅度整体缩放)               | **有** (引入了 $Sa(f)$ 衰减，高频失真)                                  |
| **恢复信号** | 低通滤波器 (LPF) 直接恢复。                         | 低通滤波器 (LPF) 直接恢复。                    | **LPF + 均衡器** (修正 $Sa(f)$ 带来的衰减)。                            |
| **实现难度** | **无法实现** (需无穷大带宽和功率)。                     | 较易实现 (模拟开关)，但在高速时能量低。                | **最易实现** (目前的标准做法，利于ADC量化)。                                  |
| **主要用途** | 纯理论分析工具。                                  | 早期多路复用系统。                            | **现代数字系统**、ADC前端。                                            |
![](Pastedimage20251229204419.png)
*   **孔径效应**
...
*   **量化噪声$e$**：由于量化是一种“四舍五入”的过程，量化后的值与原始抽样值之间存在误差 $e=y_k-x$。这个误差是随机的，被称为量化噪声。
*   **量化噪声功率$\sigma_q^2$**：用量化误差的平均功率 $\sigma_q^2$ 来描述。
...
*   **量化信噪比 $SNR_q$**：信号的平均功率与总的量化噪声（量化噪声+过载噪声）的比值。
...
*   **工作过程 (压扩)**
    1.  **压缩：在编码端，先对输入信号 $x$ 进行非线性变换 $y = f(x)$。这个变换会“压缩”大信号的幅度，同时“扩展”小信号的幅度。
    2.  **均匀量化**：对变换后的信号 $y$ 进行均匀量化。
    3.  **扩张**：在解码端，进行反变换 $\hat{x} = f^{-1}(y_k)$，恢复信号幅度。
...
#### **最佳量化器的分层电平和量化电平**
现在详细推导在最小均方误差准则下的最佳量化器的分层电平和量化电平。这个理论通常被称为**劳埃德-最大算法**的基础。
...
*   **概率密度函数 (PDF):** $p(x)$。
*   **分层电平 :** $t_0, t_1, t_2, ..., t_N$。这些值将输入信号的范围划分为 $N$ 个不重叠的区间 $R_k = (t_{k-1}, t_k]$。
*   **量化电平 :** $y_1, y_2, ..., y_N$。如果输入 $x$ 落在区间 $R_k$ 内，那么量化器的输出就是 $y_k$。
...
**结论 1 (质心条件):**
这个公式的物理意义是，**最佳的量化电平 $y_k$ 应该是输入信号在量化区间 $R_k$ 内的条件期望值，也就是该区间的概率密度函数的质心。**
...
直接设计和实现一个阶梯大小不等的非均匀量化器在硬件上是复杂的。工程上采用了一种等效且更简单的方法，称为**压扩**，这个词是**压缩**和**扩张**的组合。

流程如下：
1.  **压缩**: 在量化之前，输入信号 $x$ 先通过一个非线性压缩函数 $c(x)$。这个函数的作用是“压制”大信号的幅度，同时“拉伸”小信号的幅度。
2.  **均匀量化**: 将压缩后的信号 $c(x)$ 送入一个标准的、简单的均匀量化器进行量化。
3.  **扩张**: 在接收端，将量化后的值通过一个反函数 $c^{-1}(y)$（扩张函数）进行处理，以恢复信号的原始动态范围。
...
| **段落序号** | **段落码 (C2​C3​C4​)** | **幅度范围 (单位 Δ)** | **段内步长** |
| -------- | ------------------- | --------------- | -------- |
| **0**    | **000**             | $[0, 32)$       | **2**    |
| **1**    | **001**             | $[32, 64)$      | **2**    |
| **2**    | **010**             | $[64, 128)$     | **4**    |
...
**1.基本思想**：可以看作是 DPCM 的一种极端情况。它是一个<font color="orange">1比特</font>的DPCM系统。
*   **工作原理**：
...
**2.1 误差的统计特性**
量化误差 e(t) 的范围为 $[-\Delta, \Delta]$，假设为均匀分布，其概率密度函数 p(e) 为：

$$p(e) = \frac{1}{2\Delta}, \quad -\Delta \le e \le \Delta$$
**2.2 平均误差功率（均方值）**
计算 e(t) 的方差（总功率）：

$$E[e^2] = \int_{-\Delta}^{\Delta} e^2 \cdot p(e) \, de = \frac{1}{2\Delta} \left[ \frac{e^3}{3} \right]_{-\Delta}^{\Delta} = \frac{\Delta^2}{3}$$

**2.3 经过低通滤波器后的噪声**
假设噪声功率均匀分布在采样带宽 $f_s$ 上，接收端低通滤波器（LPF）的截止频率为 $f_m$。落在信号带宽内的噪声功率 $N_q$ 为：

$$N_q = E[e^2] \cdot \frac{f_m}{f_s} = \frac{\Delta^2 f_m}{3 f_s}$$

---

**3. 信号功率 (S) 与斜率限制**
为了获得最大 SNR，输入信号幅度应尽可能大，但受限于斜率过载条件。
**3.1 输入信号定义**
设输入单频正弦信号：

$$m(t) = A \cos(2\pi f_k t)$$
- A: 信号幅度
- $f_k$: 信号频率
- **信号平均功率**：$S = A^2 / 2$
**3.2 斜率过载条件**
- **系统最大跟踪斜率**：$k_{sys} = \Delta \cdot f_s$
    - **信号最大变化斜率**：
    $$\max |m'(t)| = \max |-A \cdot 2\pi f_k \sin(2\pi f_k t)| = 2\pi f_k A$$

**3.3 不发生过载的最大幅度 $(A_{max})$**
临界条件为 信号斜率 = 系统斜率：
$$2\pi f_k A_{max} = \Delta f_s \implies A_{max} = \frac{\Delta f_s}{2\pi f_k}$$
**3.4 最大不失真信号功率 $(S_{max})$**

$$S_{max} = \frac{A_{max}^2}{2} = \frac{1}{2} \left( \frac{\Delta f_s}{2\pi f_k} \right)^2 = \frac{\Delta^2 f_s^2}{8\pi^2 f_k^2}$$

---

**4. 最大量化信噪比公式**
将信号功率与噪声功率相除：
$$SNR = \frac{S_{max}}{N_q} = \frac{\frac{\Delta^2 f_s^2}{8\pi^2 f_k^2}}{\frac{\Delta^2 f_m}{3 f_s}}$$

化简消去 $\Delta^2$，整理得最终公式：

$$\boxed{ SNR = \frac{3}{8\pi^2} \cdot \frac{f_s^3}{f_k^2 f_m} }$$
**5.缺点与改进**
1.  **过载失真**：当输入信号的斜率（变化速率）太大，阶梯波跟不上时，会产生过载失真。
	临界条件：$max|\frac{dx(t)}{dt}| = \frac{\Delta}{t}$
2.  **颗粒噪声**：当输入信号平缓时，阶梯波会在信号上下摆动，产生颗粒噪声。
	量化噪声：$N = \frac{\Delta^2}{3}$
3.  **改进方法一：Δ-Σ调制**：在编码器输入端加一个积分器，在解码器输出端加一个微分器。这使得过载条件与信号频率无关，只与幅度有关，改善了对高频信号的处理能力。
4.  **改进方法二：自适应增量调制 (ADM)**：使量化步长 $\Delta$ 能够自适应变化。当检测到连续的'1'或'0'（表明可能过载）时，增大步长；当'0'和'1'交替出现（表明信号平缓）时，减小步长。
