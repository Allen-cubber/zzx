...
- **先拿基本盘，坚决不恋战**
    ...
    - 拿到试卷，先花 1 分钟扫一遍题目，对题目整体题量和难易程度做到心中有数。建立对部分题目绝对的信心。（如A律13折线、HDB3编码等等）
        
    - 遇到完全没见过、或者需要疯狂推导微积分的证明题（比如最佳接收机的误码率推导），直接跳过。我们的目标是通过考试，不是 100 分。
        
- **“按步骤要分”，绝不留白**
    
    - 补考阅卷老师最怕看到的就是大片空白（想给同情分都找不到地方下笔）。
    - 即便这道题不会算到最后一步，**一定要把相关的公式写上去**。比如看到“带宽”、“波特率”，就把 $B = \frac{R_B}{2} (1 + \alpha)$ 写上；看到“匹配滤波器”，就把  $h(t) = k \cdot s(t_0 - t)$ 写上。写对公式，通常就能拿到 2-3 分的步骤分。

## 2.复习战略

*  认清试卷真相，坚决“抓大放小”
	* **90% 考基础：** 期末与补考卷极少考核复杂的泛函推导或长篇证明。绝大部分题目都是基本概念、核心结论的应用以及课本上的常规计算题。
	* **什么是“大”：** A律13折线编码计算、HDB3码波形转换、升余弦滚降带宽计算、四大调制方式的性能对比表。这些必须拿下。
	* **什么是“小”：** 复杂的误码率极限公式推导、最佳接收机的底层数学证明等。遇到这类内容，直接略过推导过程，**只背最终结论公式**。

*  拒绝“眼高手低”，动手算到底
	* 像量化编码、码型转换这类纯“机械运算”的题目，复习时切忌只看不练。
	* **行动指南：** 考前务必找两道 A律编码和 HDB3 码的课后原题，盖住答案，在草稿纸上结结实实地从头算到尾两遍。只要这两遍算准了，考场上这 20~30 分就是白送的。
*  重视图形表达
	* 图形是通信工程的语言。复习时一定要亲手画一画**波形图**（AMI/HDB3 的极性跳变）和**原理框图**（PCM 编解码流程）。
	* 在解答论述题时，即使文字描述不精准，只要原理图画对，通常也能拿到可观的分数。
...
阅卷老师评阅简答题主要看**关键词**。只要踩中以下触发词，分就拿到一半。

* **问：为什么用 2DPSK 而不用 2PSK？**
    * **必写关键词：** 克服**相位模糊**。
* **问：引入升余弦滚降滤波器的目的是什么？**
    * **必写关键词：** 消除**码间串扰**，且物理上易于实现。
* **问：增量调制 ($\Delta$M) 会产生哪两种噪声？**
    * **必写关键词：** **“一般量化噪声”**（或颗粒噪声）、**“斜率过载噪声”**。
...
### 1. HDB3 码的“双重自检法”
画完波形或写出码字后，回头复查两点：
* **一重检查：** 所有的 $V$ 脉冲（破坏点），极性是否严格正负交替？（$+V, -V, +V \dots$）
* **二重检查：** 除了 $V$ 脉冲之外的所有非零脉冲（不管是原来的信息码 $1$ 还是补齐用的 $B$ 脉冲），极性是否严格正负交替？

### 2. 单位陷阱
* **信息速率 ($R_b$)** $\rightarrow$ 单位必须写 **bps** 或 **b/s**。
* **码元速率/波特率 ($R_B$)** $\rightarrow$ 单位必须写 **Baud**。
* **带宽 ($B$)** $\rightarrow$ 单位必须写 **Hz**。
* *提醒：多进制计算时，千万别忘了公式 $R_b = R_B \log_2 M$。*
