...
## 附录一 恶性码
### 核心概念
**恶性码的定义**：如果在信道中发生了**有限个**比特的错误，却导致译码器输出**无限个**比特的错误，这种码就是恶性码。

---

### 1. 构造一个恶性码的例子

假设我们有一个 $(2, 1, 2)$ 卷积码（即：1位输入，2位输出，约束长度为3），其生成多项式设计如下：

*   **输出路 1**：$g_1(D) = 1 + D$
*   **输出路 2**：$g_2(D) = 1 + D^2$

> **⚠️ 注意**：在二进制域 GF(2) 中，$1+D^2 = (1+D)(1+D)$。
> 你会发现这两个多项式有一个**公因式** $(1+D)$。**这就是导致它成为恶性码的罪魁祸首。**

### 2. “灾难”是如何发生的？

我们来看一个特定的输入序列：**无限长的全 1 序列**。

#### 第一步：定义输入
假设输入信息序列 $u$ 是无限个 1：$1, 1, 1, 1, 1, \dots$
在多项式中，全 1 序列可以表示为：
$$ u(D) = 1 + D + D^2 + D^3 + \dots = \frac{1}{1+D} $$
*(注：类似于几何级数求和)*
...
*   **第一路输出 $v_1(D)$**：
    $$ v_1(D) = u(D) \cdot g_1(D) = \frac{1}{1+D} \cdot (1+D) = 1 $$
    对应序列：**1, 0, 0, 0, 0...** （只有一个 1）

*   **第二路输出 $v_2(D)$**：
    $$ v_2(D) = u(D) \cdot g_2(D) = \frac{1}{1+D} \cdot (1+D^2) = \frac{1}{1+D} \cdot (1+D)(1+D) = 1+D $$
    对应序列：**1, 1, 0, 0, 0...** （只有两个 1）

#### 第三步：传输与灾难
*   **编码器的实际输出**（将 $v_1$ 和 $v_2$ 交织）：
    $$(1,1), (0,1), (0,0), (0,0), \dots$$
    也就是：**11, 01, 00, 00, 00...**

可以看到，虽然输入了**无数个 1**，但输出只有**3个 1**，后面全是 0。

**💥 灾难场景：**
假设在传输过程中，信道只发生了**3个比特的错误**（有限个错误），刚好把开头的 `11` 和 `01` 里的 `1` 全部变成了 `0`。

1.  **接收端收到**：`00, 00, 00, 00, ...` （全零序列）。
2.  **译码器判断**：根据最大似然准则，收到全零，译码器最自然会判定输入是“全零序列” ($0, 0, 0, \dots$)。
3.  **结果**：
    *   **实际输入**：$1, 1, 1, 1, \dots$ (无限个1)
    *   **译码输出**：$0, 0, 0, 0, \dots$ (无限个0)
    *   **误码率**：信道只错了3位，译码后却错了无穷多位！

### 3. 图解状态图 (State Diagram)

如果画出这个码的状态图，你会发现一个致命的结构特征：

在这个状态图中，存在一个**自环（Self-loop）**，对应的**输入是非零**的，但**输出却是零**（或输出权重极小）。

*   当编码器进入这个状态循环时，它不断地吃进“1”，却不断地吐出“0”。
*   一旦在这个循环路径上，距离全零路径（正确路径）的距离（汉明距离）不再增加。
*   一旦噪声把进入循环前的那一点点非零输出抹掉，译码器就再也分不清是在“全零路径”上还是在“全1路径”上了。

### 4. 为什么“系统结构卷积码”是非恶性的？

回到你上一道题的结论：**系统码一定不是恶性码**。

因为系统码的输出直接包含了输入：
*   **输入**：$u(D)$
*   **输出**：$(u(D), p(D))$

如果输入有无限个 1（权重为无穷大），那么输出的第一部分也就是无限个 1。
无论生成多项式怎么变，**输出的权重一定也是无穷大**。

要想在接收端让这“无穷大”权重的输出变成全 0，信道必须产生**无穷多**的错误位。这就打破了恶性码“有限错误导致无限误差”的定义。

### 总结
*   **恶性码特征**：生成多项式之间有公因式（如 $1+D$）。
*   **表现**：输入“重如泰山”（无限重），输出“轻如鸿毛”（有限重）。
*   **后果**：信道稍有风吹草动（几个误码），解码器就会彻底迷失方向（无限误码）。