 ...
 *   **分贝(dB):** 定义为两个功率的比值，取常用对数后，再乘以10或20。
  ...
  *   **级联放大器的噪声系数**：其中 $F_{ni}$ 是第i级的噪声系数，$G_{pi}$ 是第i级的额定功率增益。
  * **结论:** 🥇 多级放大器的总噪声系数主要取决于第一级的噪声系数 $F_{n1}$ 和额定功率增益 $G_{p1}$。要降低总噪声系数，关键是降低第一级的噪声系数 $F_{n1}$ 和提高第一级的功率增益 $G_{p1}$。$\boxed{ F_n = F_{n1} + \frac{F_{n2}-1}{G_{p1}} + \frac{F_{n3}-1}{G_{p1}G_{p2}} + \dots + \frac{F_{nN}-1}{G_{p1}G_{p2}\dots G_{p(N-1)}} }$
...
*   **(1) 放大器输入导纳 $Y_i$:** 📥
    *   考虑 $y_{re}$ 时:
        $$
         Y_i = y_{ie} - \frac{y_{re} y_{fe}}{y_{oe} + Y''_L}
        $$
...
*   **(2) 放大器输出导纳 $Y_o$:** 📤
    *   考虑 $y_{re}$ 时:
        $$
        \boxed{ Y_o = y_{oe} - \frac{y_{re} y_{fe}}{Y_s + y_{ie}} }
        $$
    *   其中 $Y_s$ 是信号源内导纳。
...
*   **(3) 电压增益 $A_v$:** 📈 (参考 P70)
    ...
    *   电压增益表达式:
        $$
        A_v = \frac{-p_1 p_2 y_{fe}}{g_\Sigma [1 + j 2Q_L \frac{\Delta f}{f_0}]}
        $$
    *   **谐振时电压增益:**
        $$
...
*   **(4) 功率增益 $G_p$:** 💪 (参考 P71)
    ...
    *   **最大功率增益 (匹配时):**
        $$
        (G_{po})_{\text{max}} = \frac{|y_{fe}|^2}{4 g_{oe} g_{ie}}
        $$
    *   **插入损耗 $K_1$:** 考虑回路自身损耗 $G_o$ 引起增益下降的因素。
        $$
        K_1 = \frac{P_o}{P_o'} = (1 - \frac{G_o}{g_\Sigma})^{-2} = (1 - \frac{Q_L}{Q_o})^{-2}
        $$
        (功率比，表示损耗带来的影响， $P_o'$ 为无损耗时功率)
...
**1. 单级双调谐回路谐振放大器:** (参考 P79)
   ...
   *   **耦合系数 $\eta$:**
      *   定义:
          $$
          \boxed{\eta = \frac{k}{k_{crit}} = \frac{M/ \sqrt{L_1 L_2}}{1/\sqrt{Q_1 Q_2}} = k \sqrt{Q_1 Q_2}}
          $$
          （对于并联回路， $k=M/\sqrt{L_1 L_2}$）
...
**2. 双调谐回路性能 (临界耦合 $\eta=1$ 时):** ✨
   *   **(1) 电压增益:**
      *   谐振时最大:
          $$
          \boxed{ A_{v0, \eta=1} = \frac{-p_1 p_2 |y_{fe}|}{2g} }
          $$
          (g 为单边回路等效电导, $g = g_\Sigma/2$)
   *   **(2) 通频带:**
      *   $$
...
### 🌟 考点速记
1. 高频小信号放大器特点：**工作在线性区、具备选频能力**、噪声系数极低、兼顾增益与稳定性。
2. 参数定义：
	  *   **电压增益:**
	     $\boxed{ A_v = \frac{\dot{V}_o}{\dot{V}_i} }$ 或 $A_v (\text{dB}) = 20 \lg |A_v|$
	  *   **功率增益:**
	  ...
	  *   **矩形系数:** 衡量谐振曲线接近理想矩形程度的指标。
	      $\boxed{ K_{r0.1} = \frac{BW_{0.1}}{BW_{0.7}} = \frac{2\Delta f_{0.1}}{2\Delta f_{0.7}} }$
	  *   **噪声系数 ($F_n$):** 放大器输入端信噪比与输出端信噪比的比值。
		 $\boxed{ F_n = \frac{(P_{si}/P_{ni})}{(P_{so}/P_{no})} }$ 
...
3.  晶体管的频率参数: 
	   *   **(1) 截止频率 $f_\beta$ :**
	      *   定义：共发射极电路**电流放大系数** $|\beta|$ 下降到其低频值 $\beta_0$ 的 $\frac{1}{\sqrt{2}} \approx 0.707$ 倍时的频率。 $\boxed{ f_\beta }$
	   *   **(2) 特征频率 $f_T$ :**
	      *   定义：共发射极电流放大系数 $|\beta|$ 下降到 1 时的频率。 $\boxed{ f_T }$ 估算：$|\beta| \approx f_T / f$ (当 $f \gg f_\beta$)。
	   *   **(3) 最高振荡频率 $f_{max}$ :**
	      *   定义：晶体管的**功率增益** $G_p$ 下降到 1 (0dB) 时的频率。 $\boxed{ f_{max} }$ 实际工作频率通常选取为 $(1/3 \sim 1/4) f_{max}$。
4. 单调谐回路谐振放大器
	* 典型**共射极**高频调谐放大器电路。
	* **接入系数 :** 
	    *   晶体管接入回路: $p_1 = N_1/N$ (抽头)
	...
	* 谐振电压增益：$$
        ...
        $$
    * **插入损耗 $K_1$:** 考虑回路自身损耗 $G_o$ 引起增益下降的因素。$$
        \boxed{K_1  = (1 - \frac{Q_L}{Q_o})^{-2}}
        $$
    * **通频带** $BW_{0.7}$:
	    由并联谐振回路理论: $\boxed{ BW_{0.7} = 2\Delta f_{0.7} = \frac{f_0}{Q_L} }$
	* **选择性** (矩形系数 $K_{r0.1}$):
	     $\boxed{ K_{r0.1} = \frac{\Delta f_{0.1}}{\Delta f_{0.7}} = \sqrt{10^2-1} / \sqrt{(\frac{1}{0.707})^2-1} = \sqrt{99} \approx 9.95\ \ }$
5. 多级单调谐回路谐振放大器
	* 电压增益: 
	$$A_m = (A_{v1})^m$$
	* 通频带: 
    $$BW_m = BW_1 \sqrt{2^{1/m}-1}$$
	* 选择性:
    $$K_{r0.1,m} = K_{r0.1,1} \frac{\sqrt{10^{2/m}-1}}{\sqrt{99(2^{1/m}-1)}}$$
    
	- 级联可以改善选择性（$K_{r,m}$ 随 m 增大而减小，趋近于1），但当 $m>3$ 时改善效果不明显。

6. 双调谐回路谐振放大器
	* **目的:** 改善选择性，缓和增益与通频带的矛盾。
	- **谐振最大电压增益:**
	    $$A_{v0, \eta=1} = \frac{-p_1 p_2 |y_{fe}|}{2g}$$
	- **通频带:**
	    $$BW_{0.7, double} = \sqrt{2} \frac{f_0}{Q_L} = \sqrt{2} \cdot BW_{0.7, single}$$
	- **选择性 (矩形系数):**
	    $$K_{r0.1, \eta=1} \approx 3.16$$
	- **总通频带 ($m$ 级):**
	    $$BW_m = BW_{1, double} \sqrt[4]{2^{1/m}-1}$$
	- **选择性 ($m$ 级矩形系数):**
	    $$K_{r0.1,m} = \frac{\sqrt[4]{10^{2/m}-1}}{\sqrt[4]{2^{1/m}-1}}$$
7. 稳定性与稳定措施
	* $y_{re}$ 影响稳定性，可能导致放大器工作不稳定，甚至自激振荡。
	* 实现晶体管单向化的方法：失配法（共射-共基级联电路）、中和法（加一个反馈电路）。

### 🌟 本章总结

本章主要学习了高频小信号放大器的基本概念、性能指标、分析方法和典型电路。
1.  **核心概念（定性掌握）:** 了解了高频窄带放大器的特点（选频、线性工作）及其在通信系统（如超外差接收机）中的作用。
2.  **性能指标（定量计算）:** 掌握了增益（电压/功率）、通频带、选择性（矩形系数）、工作稳定性和噪声系数的定义和计算方法，特别是级联噪声系数的计算及其意义。
3.  **等效电路与参数（定性掌握）:** 学习了晶体管在高频下的两种主要等效电路（混合π模型和y参数模型），理解了关键参数（$C_{b'c}$, $r_{bb'}$, y参数, $f_\beta$, $f_T$, $f_{max}$）的物理意义和影响。
4.  **单调谐放大器（定量计算）:** 详细分析了单调谐回路放大器的输入/输出导纳、电压/功率增益、通频带和选择性。认识到其主要缺点是选择性差以及增益与带宽的矛盾。
5.  **多级单调谐放大器（定量计算）:** 了解了级联对增益、通频带（缩窄）和选择性（改善）的影响。
6.  **双调谐放大器（定量计算）:** 学习了双调谐回路通过耦合系数 $\eta$ 改善选择性和带宽的原理。重点掌握了临界耦合状态下的性能分析，其带宽是同Q值单调谐的 $\sqrt{2}$ 倍，选择性更好。
7.  **稳定性问题（定性掌握）:** 理解了晶体管内部反馈参数 $y_{re}$ 对放大器稳定性的影响（互作用、自激风险）。
8.  **稳定措施（定性掌握）:** 掌握了两种提高稳定性的主要方法：失配法（特别是Cascode电路的应用）和中和法，了解了它们的原理、优缺点和实现方式。
...
如果要使瞬时频率 $\omega(t) = \omega_0 + k_f v_\Omega(t)$，则瞬时相位 $\theta(t) = \omega_0 t + k_f \int v_\Omega(t)dt + \theta_0$。
若使用调相器，其瞬时相位为 $\theta(t) = \omega_0 t + k_p v'_\Omega(t) + \theta_0$。
要使调相器输出为调频波，则需要 $k_p v'_\Omega(t) = k_f \int v_\Omega(t)dt$。
这意味着，如果将调制信号 $v_\Omega(t)$ 先积分，得到 $\int v_\Omega(t)dt$，然后将其作为调相器的调制输入 $v'_\Omega(t)$，则调相器的输出就是一个调频波。
...
##### 3. 原理分析
设输入调频信号 $a(t) = V_0 \cos(\omega_0 t + m_f \sin(\Omega t)) = V_0 \cos(\theta_0(t))$
