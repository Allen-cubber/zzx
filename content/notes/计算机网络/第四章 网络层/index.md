...
> **网络层的任务**：把许多不同的网络连接起来，让主机之间能够跨网络通信。
> **虚拟互联网络**：利用 IP 协议把各种异构网络互连起来，屏蔽底层网络差异，使用户看起来好像在一个统一的网络中通信。
### 4.1.1 网络层提供的两种服务 🛤️
...
- **D类地址**：以 $1110$ 开头，是多播地址，224.0.0.0 ~ 239.255.255.255不能分配给主机。
    
- **E类地址**：以 $1111$ 开头，保留为今后使用 。

| 类别  |   第一个字节范围 | 地址范围                        | 默认用途    |
| --- | --------: | --------------------------- | ------- |
...
| D 类 | 224 ~ 239 | 224.0.0.0 ~ 239.255.255.255 | 多播 / 组播 |
| E 类 | 240 ~ 255 | 240.0.0.0 ~ 255.255.255.255 | 保留实验    |

⚠️ **网络前缀**是指“整个的网络号字段”，而不是仅仅指前几个类别位。

⚠️ **特殊 IP 地址注意点**：

| **特殊 IP 形式**  | **举例**            | **能否作为源 IP**       | **能否作为目的 IP**  | **作用**                   |
| ------------- | ----------------- | ------------------ | -------------- | ------------------------ |
| **全 0**       | `0.0.0.0`         | **能** (仅限未分配 IP 时) | **不能** (路由表除外) | 代表未指定地址 / 默认路由           |
| **全 1**       | `255.255.255.255` | 不能                 | **能**          | 本地局域网受限**广播**            |
| **网络号 + 全 0** | `192.168.1.0`     | 不能                 | 不能             | 代表整个网段，用于路由表，它不代表任何单台主机。 |
| **网络号 + 全 1** | `192.168.1.255`   | 不能                 | **能**          | 对特定网段的所有主机进行直接广播         |
| **127 网段**    | `127.0.0.1`       | **能**              | **能**          | 本地软件环回测试，不出网卡            |

在一个具体网络中，通常不能分配给主机的是：

| 类型           | 说明                |
| ------------ | ----------------- |
| 网络地址         | 主机号全 0            |
| 广播地址         | 主机号全 1            |
| D 类地址        | 组播地址，不能作为普通主机单播地址 |
| E 类地址        | 保留地址              |
| 127.x.x.x    | 回环地址              |
| 每段超过 255 的地址 | 非法 IPv4 地址        |
...
- **标志 (flag)**：占3位 。
    ...
    - $DF=0$ 时才允许分片 。**当 `DF = 1` 时：** 意思是“绝对不能对这个包进行分片” 。如果这个包裹太大了，路由器不会去拆它，而是**直接把这个包丢弃**，然后给发送端回传一条 ICMP 报错消息。
        
- **片偏移**：占13位。指出分片后在原分组中的相对位置。**以8个字节为偏移单位**，所以分片长度必须是8的整数倍。 
    
- **生存时间 (TTL)**：占8位。数据报在网络中可通过的路由器数的最大值 。数据报每通过一个路由器，其内部的TTL值将减1。
    
- **协议**：占8位。指出当目标电脑的 IP 层（网络层）收到并拆开这个数据报后，决定把里面装的实际数据（Payload）交给上层的哪一个具体协议去处理（如 ICMP=1, TCP=6, UDP=17, OSPF=89 等） 。
    
- **首部检验和**：占16位。**只检验数据报的首部**，不检验数据部分。数据报每经过一个路由器都要重新计算 。（因为IP 首部里 **TTL（生存时间）** 的字段，数据包每经过一个路由器，路由器都必须把 TTL 减 1，那么原来算好的“首部检验和”就作废了。）
    

**发送方（比如你的电脑）的工作流程：**

1. 先把这个 16 位的“首部检验和”字段全部填成 `0`。
    
2. 把整个 20 字节的 IP 首部，切成 10 个 16 位（2字节）的数据块。
    
3. 把这 10 个数据块全部加起来。如果相加的时候发生了最高位“溢出”（超出了 16 位），就把溢出的那个 `1` 拿回来，加到最低位上。
    
4. 把最终得到的求和结果**取反**（0变1，1变0），填入“首部检验和”字段。数据包发送出列。
    

**接收方（或中间的路由器）的工作流程：**

1. 收到数据包后，把首部同样的 10 个 16 位数据块（这次包含了发送方算好的检验和）全部加起来。
    
2. 如果传输中**没有任何错误**，最终加出来的结果一定全是 `1`（即十六进制的 `FFFF`）。
    
3. 如果结果里有一个 `0`，说明首部在半路上被破坏了，路由器二话不说，**直接丢弃这个包**。
...
- **判断是否直连：** 若 $D$ 属于某个直连网络网段，则通过 **ARP** 直接交付；否则，进入间接交付，查找转发表。
    
- **查特定主机路由：** 若转发表中有目的地址为 $D$ 的特定主机路由，则转发给对应的下一跳；否则，继续。
    
- **查最长前缀路由：** 按前缀从长到短顺次匹配转发表。若匹配成功，转发给对应的下一跳；否则，继续。
    
- **查默认路由：** 若有默认路由，转发给默认网关；否则，**丢弃分组并报告 ICMP 差错**。
...
- 规则：先检查 IP 地址左边的第一位，如为 0，则第一层的节点就在根节点的左下方；如为 1，则在右下方。然后再检查地址的第二位，构造出第二层的节点。依此类推，直到唯一前缀的最后一位。每个叶节点代表一个唯一前缀 。注意**并非只有叶子节点才是前缀，内部节点也可以是前缀**。
...
| 255.255.255.254 | 222.13.16.4（R1-R3链路） | 无            | m1  |
...
- **类型（Type） - 1 字节**： 用来标识这是什么类型的 ICMP 报文。例如：
    
    - `8`：回送请求（Ping Request）
        
    - `0`：回送回答（Ping Reply）
        
    - `3`：终点不可达（Destination Unreachable）
        
    - `11`：时间超过（Time Exceeded，通常是 TTL 耗尽）
        
- **代码（Code） - 1 字节**： 用来进一步细分“类型”。比如在“终点不可达”（类型 3）中，代码 `0` 表示网络不可达，代码 `1` 表示主机不可达，代码 `3` 表示端口不可达。
    
- **检验和（Checksum） - 2 字节**： 用于检查整个 ICMP 报文（包括首部和数据部分）在传输过程中是否出现了错误。
    
- **首部剩余部分（Rest of Header） - 4 字节**： 这 4 个字节的内容完全取决于前面的“类型”和“代码”。
    
    - 如果是 Ping 请求/应答，这部分会被拆分为 **标识符（2字节）** 和 **序号（2字节）**，用来匹配请求和响应。
        
    - 如果是差错报告报文（如终点不可达），这 4 个字节通常是 **未使用（全填 0）**。
...
- **为什么要提取这前 8 个字节？** 因为 IP 数据报的数据字段里面通常装的是传输层报文（如 TCP 或 UDP）。TCP/UDP 报文的前 8 个字节恰好包含了**源端口号**和**目的端口号**。源主机收到退信后，一看端口号，就知道是自己电脑上的哪个软件（比如浏览器还是微信）发出的请求失败了。
...
## 4.5 <font color="orange">IPv6</font>（了解）
...
- **数据报格式：** 
	* 由两大部分组成：**基本首部** (base header) 和 **有效载荷** (payload，也称为净负荷) 。
...
| **比较维度**         | **🧮 IPv4**                               | **🚀 IPv6**                                              | **💡 带来的好处 / 意义**                                 |
| ---------------- | ----------------------------------------- | -------------------------------------------------------- | ------------------------------------------------- |
| **地址长度**         | 32 位 (约 43 亿个)                            | 128 位 (极其海量)                                             | **解决地址枯竭**。IPv6 的地址多到可以给地球上的每一粒沙子都分配一个 IP。        |
| **表示方法**         | 点分十进制<br><br>  <br><br>_(例: 192.168.1.1)_ | 冒号十六进制<br><br>  <br><br>_(例: 2001:0db8::8a2e:0370:7334)_ | 适应更长的地址，同时 IPv6 支持“零压缩”等简写方式。                     |
| **首部(Header)大小** | 20 字节（可变长，最长 60 字节）                       | 40 字节（固定长度）                                              | 固定长度的首部让路由器处理数据包时**速度更快**，不需要额外计算首部长度。            |
| **首部结构**         | 复杂（包含选项字段、头部校验和等）                         | 精简（去掉了校验和、分片相关字段，移入扩展首部）                                 | **提高转发效率**。IPv6 取消了路由器每转发一次都要重新计算校验和的繁琐步骤。        |
| **地址配置**         | 通常需要 DHCP 服务器分配或手动配置                      | 支持 **SLAAC**（无状态地址自动配置）                                  | **即插即用**。设备连入 IPv6 网络后，可以直接根据路由器的广播自动生成自己的 IP 地址。 |
| **安全性 (IPsec)**  | 可选附加项，需额外配置                               | 设计之初就内置集成 (原生支持)                                         | IPv6 在网络层面上提供了更好的端到端加密和身份验证基础。                    |
| **广播功能**         | 支持广播 (Broadcast)                          | **取消了广播**，采用多播 (Multicast) 和任播 (Anycast) 替代              | 广播会打扰局域网内所有设备，取消广播**大幅降低了网络噪音**，提高了带宽利用率。         |

---

## 4.6 <font color="orange">互连网的路由选择协议</font>（重点）

### 4.6.1 <font color="orange">有关路由选择协议的几个基本概念</font>

- **控制层面与数据层面：** 路由选择协议属于网络层的**控制层面**（通过路由选择算法构建转发表），而实际的数据转发动作属于**数据层面** 。
    
- **理想的路由算法：** 包含正确完整性、简易性（计算简单）、稳健性（自适应）、稳定性（收敛）、公平性、合理性（最佳）等特点 。
...
> 常见路由选择协议有 RIP、OSPF 和 BGP，其中 RIP 采用距离向量算法，OSPF 采用链路状态算法，BGP 采用路径向量算法。
---

### 4.6.2 <font color="orange">内部网关协议 RIP 距离向量算法</font>
...
- **RIP2 报文格式：** 使用 **UDP** 传送（端口 520） 。一个报文最多可包括 25 个路由，最大长度为 $4+20\times25=504$ 字节 。支持无分类域间路由选择 CIDR 和简单的鉴别功能 。$$\text{结构：[帧头][IP头][UDP头][RIP报文内容]}$$
- **💡 注意点（主要缺点）：** **坏消息传播得慢**（慢收敛）。当网络出现故障时，需要经过较长时间才能将此信息传送到所有路由器 。
    

---

### 4.6.3 <font color="orange">内部网关协议 OSPF 链路状态算法</font>

- **基本原理：** 开放最短路径优先 OSPF (Open Shortest Path First) 为了克服 RIP 的缺点而开发，使用 Dijkstra 的**最短路径**算法，采用分布式的**链路状态协议** 。
...
- **三个主要特点：**
	...
	* **注：洪泛法**强调的是“信息的完整复制与扩散”，最终使得全网所有节点都得到一张**一模一样的全网拓扑大地图**。而对于 RIP 协议那种一级一级加工、修改、选择性告知的传递方式，就绝对不能叫洪泛法。
...
- **OSPF 分组：** 共有 5 种（问候、数据库描述、链路状态请求、链路状态更新、链路状态确认），直接使用 **IP 数据报** 传送（协议字段值为 89） 。$$\text{结构：[帧头][IP头][OSPF报文内容]}$$
    
- **收敛速度：** 链路状态数据库在全网范围内同步后，OSPF 没有“坏消息传播得慢”的问题，收敛速度快 。
    
| **路由协议** | **核心算法**  | **跟谁交换信息？** | **交换什么信息？** | **怎么交换？**          | **最终数据库状态**        |
| -------- | --------- | ----------- | ----------- | ------------------ | ------------------ |
| **RIP**  | 距离向量 (DV) | **仅相邻的邻居**  | 自己的整个路由表    | 定期异步交换             | 节点间无统一全网拓扑         |
| **OSPF** | 链路状态 (LS) | **全网所有节点**  | 与自己相邻的链路状态  | **洪泛法 (Flooding)** | **全网拥有完全相同的 LSDB** |

---

### 4.6.4 <font color="orange">外部网关协议 BGP 路径向量算法</font>
...
- **连接方式：** 在半永久性 **TCP 连接**（端口号 179）上建立会话 。跨 AS 的连接称为 eBGP 连接，AS 内部的连接称为 iBGP 连接 。eBGP 解决了“AS 与 AS 之间怎么连”的问题，而 iBGP 解决了“在同一个 AS 内部，如何安全、高效、不丢失属性地传递境外路由”的问题。$$\text{结构：[帧头][IP头][TCP头][BGP报文内容]}$$
    
- **BGP 路由信息：** 包含前缀和 BGP 属性。最重要的两个属性是 **AS-PATH**（自治系统路径）和 **NEXT-HOP**（下一个 AS 边界路由器的 IP） 。
    
- **🚫 易错点（防环）：** 收到 BGP 路由时，若发现 AS-PATH 中**已经有自己所在 AS 的号码**，则立即删除该路由 。
...
| **路由协议** | **包装选择** | **这样做的核心优点（工程考量）**                                                                                                                                        |
| -------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **RIP**  | **UDP**  | **简单、开销小。** RIP 用于小型局域网，路由表最多只有 15 跳。UDP 没有三次握手，不需要维护连接状态，直接广播或组播发出去，对早期低端路由器的 CPU 和内存极其友好。                                                               |
| **OSPF** | **裸 IP** | **极度追求效率，减少开销。** OSPF 用于大型自治系统（如整个校园网/企业网），要频繁传输复杂的“地图碎片（LSA）”。绕过传输层，可以省下 8 字节（UDP）或 20 字节（TCP）的头部空间，让数据包更轻量。同时，OSPF 自己在内部实现了一套硬核的确认机制（LSAck），不需要 TCP 帮忙。 |
| **BGP**  | **TCP**  | **追求极致的绝对可靠性。** BGP 管理的是全球因特网。现在全球 BGP 路由表有接近 100 万条跨国路由。这么庞大的资产库，要是中间丢一个包或错一个比特，整个国家的网络可能都会瘫痪。BGP 索性把“不丢包、防乱序、大包分段”的重活全部外包给完美的 **TCP**，自己专心处理核心的政治选路逻辑。  |
...
## 4.7 <font color="orange">IP 多播</font>（了解）
...
- **核心优势：** 当多播组的主机数很大时，采用多播方式只需发送**一次**数据到多播组，由**路由器负责复制分组**，这可明显地减轻网络中各种资源的消耗。局域网具有硬件多播功能，甚至不需要路由器复制分组 。
    ![[Pastedimage20260421102239.png]]
    - **运行机制：** 在互联网范围的多播要靠路由器来实现，能够运行多播协议的路由器称为**多播路由器**（也可以转发普通的单播 IP 数据报） 。在互联网上进行多播就叫做 **IP 多播**。
...
- IANA（互联网号码分配局）拥有的以太网地址块的高 24 位为 `00-00-5E` 。
...
- **三种常用转发方法：**
    ...
    1. **洪泛与剪除：**
        ...
        - **RPB (反向路径广播)：** 为避免数据兜圈子，路由器收到多播分组时，**先检查它是否是经过最短路径传送来的**（查找单播路由表）。如果是，则转发；如果不是，则直接丢弃！
        * 配合**剪枝**（剪除没有组成员的下游树枝）和**嫁接**（新成员加入时重新接入树枝）机制 。
...
- 采用专用IP地址的互连网络称为**专用网**，专用IP地址也称为可重用地址 。
