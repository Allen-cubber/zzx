...
| **特度维度**  | **UDP 应用 (DNS, 流媒体, DHCP)** | **TCP 应用 (HTTP, FTP, Email)** |
| --------- | --------------------------- | ----------------------------- |
| **核心诉求**  | 速度快、低延迟、广播、轻量化              | 数据100%准确、顺序无误、稳定              |
| **数据包大小** | 通常是小数据包（一问一答）               | 通常是连续的大规模数据流                  |
| **容错态度**  | “丢了就算了，接着看下一个”              | “必须重传，直到完全正确为止”               |
| **连接状态**  | 随拿随放，发完即走（无连接）              | 必须先打招呼建立管道（面向连接）              |
...
**端口的设定**： 为了解决由于进程的创建和撤销都是动态的而导致发送方无法直接识别进程的问题，运输层把**端口**设为通信的抽象终点 。

- **软件端口与硬件端口**：软件端口是协议栈层间的抽象的协议端口，是应用层进程与运输实体进行层间交互的地点；而硬件端口是不同硬件设备进行交互的接口 。运输层所说的端口指的是软件端口。
...
1. **服务器端使用的端口号**：
    ...
    - **登记端口（用户端口）**：$1024 \sim 49151$，需在 IANA 登记 。
        
2. **客户端使用的端口号**：
    
    - **短暂端口（动态端口）**：$49152 \sim 65535$，通信结束后，被系统收回 。
...
⚠️ **易错点提醒**：应用程序必须选择合适大小的报文。若报文太长，IP 层在传送时可能要进行分片（极不推荐IP分片），降低 IP 层的效率，增加丢包率；若报文太短，会使 IP 数据报的首部的相对长度太大，同样降低 IP 层的效率 。
...
2. **目的端口**：终点交付报文时必须使用 。UDP 接收方正是根据首部中的目的端口号，把报文通过相应的端口上交给应用进程；如果发现目的端口号**不正确**（也叫不可达，指接收方主机的操作系统中，当前没有任何一个进程在监听或绑定这个目的端口），就丢弃该报文，并由 ICMP 发送“端口不可达”差错报文给发送方 。
    
3. **长度**：UDP 用户数据报的长度，其最小值是8（仅有首部） 。
    
4. **检验和**：检测 UDP 用户数据报在传输中是否有错。如果检验和计算发现错误，接收方会直接**丢弃报文**。
...
![](Pastedimage20260522151446.png)

**什么是“伪首部”（Pseudo-Header）？**

按理说，UDP 运输层只管自己的 UDP 首部和数据。但在计算检验和时，UDP 协议会临时从网络层（IP层）“借”来 12 个字节的数据，拼在自己前面。这 12 个字节构成的就是“伪首部”。
...
- 协议号 (1字节，UDP的协议号固定为 17)
    
- UDP 长度 (2字节，就是 UDP 首部中的那个长度)

**为什么要加伪首部？** 这是为了“双重保险”。UDP 把数据交给了特定的端口，但万一底层的 IP 层发生错误，把数据报送错了机器（目的 IP 错了），或者本该是 TCP 的包错送给了 UDP（协议号错了）怎么办？加入伪首部计算检验和，可以让接收方的 UDP 层在校验时，顺便核对一下：“这包数据**确实是发给我的 IP 地址的，并且确实是给 UDP 协议的**”。

**“16位二进制反码求和”怎么算？**

这里的加法和我们平时的十进制加法，甚至普通的二进制加法都有一个关键区别：**最高位产生的进位不能丢弃，必须加回最低位（这叫“回卷”，Wrap-around）。**
...
⚠️**如果计算出的检验和结果是全 0，必须将其强行修改为全 1 发送**。在反码算术中，全 1（$-0$）和全 0（$+0$）的数学意义是完全等价的，但这样就完美避开了全 0，既完成了校验，又不会让接收方产生误解。

| **错误类型**      | **接收方的动作**               | **发送方能收到 ICMP 报错吗？** | **为什么？**                                   |
| ------------- | ------------------------ | -------------------- | ------------------------------------------ |
| **UDP 检验和出错** | **直接丢弃** (Silently Drop) | **❌ 不能**             | UDP 是不可靠传输，且错包的数据不可信，发 ICMP 反而浪费网络带宽。      |
| **目的端口不可达**   | **丢弃 + 触发 ICMP**         | **✓ 能**              | 报文本身是健康的，说明网络送达没问题。通知发送方可以帮它及时断开无效连接或停止盲发。 |
## 5.3 传输控制协议 TCP 概述 🌐

### 5.3.1 TCP 最主要的特点 🌟

- **连接导向**：TCP 是面向连接的运输层协议 。在无连接的、不可靠的 IP 网络服务基础之上提供**可靠交付**的服务 。
...
- TCP 把连接作为**最基本的抽象** 。“抽象”在计算机里的意思就是：**屏蔽底层的复杂混乱，给你一个干净、简单的使用界面。**
    
- 每一条 TCP 连接有两个端点 。这个端点被叫作**套接字** (socket) 或插口 。TCP 连接的终点是套接字。
...
📌 **注意点**：TCP 连接就是由协议软件所提供的一种抽象 。同一个 IP 地址可以有多个不同的 TCP 连接 ；同一个端口号也可以出现在多个不同的 TCP 连接中 ，但一个端口只能被一个进程独占。
...
### 5.4.1 停止等待 ARQ（ Automatic Repeat request ） 协议 ⏳
...
- **2. 出现差错**：
    ...
    - **解决方法：超时重传** 。A 为每一个已发送的分组设置一个**超时计时器** 。只要在超时计时器到期之前收到了相应的确认，就撤销该超时计时器，继续发送下一个分组 。若在规定时间内没收到确认，就认为分组错误或丢失，重发该分组 。
        
- **3. 确认丢失和确认迟到**：
    
    - **确认丢失**（ACK丢失）：B 发送的ACK丢失，A 超时后重传。B 收到重复分组后采取两个行动：(1) 丢弃这个重复的分组，不向上层交付 ；(2) 向 A 发送确认 。
        
    - **确认迟到**（ACK迟到）：B 的ACK迟到，A 超时重传。B 收到重复分组后丢弃，并重传确认分组 。A 收到重复的确认也会作丢弃处理 。
...
4. **数据偏移**：占 $4$ 位，指出 TCP 报文段的数据起始处距离 TCP 报文段起始处有多远。它的单位是 $32$ 位字，也就是以 **$4$ 字节**为计算单位。数据偏移实际上就是 <font color="orange">TCP 首部长度</font>。最小值为 $5$ ,最大值为 $15$ 。
    
5. **保留**：占 $6$ 位，保留为今后使用，但目前应置为 $0$。
    
6. **窗口**：占 $2$ 字节。窗口值告诉对方：从本报文段首部中的确认号算起，接收方目前允许对方发送的数据量，单位是字节。**确认号**负责确认滑动窗口的 **左边界（后沿）**，而**窗口值**负责确认滑动窗口的 **右边界（前沿）**。
...
| 控制位   | 名称  | 含义                                      |
| ----- | --- | --------------------------------------- |
| $URG$ | 紧急  | 紧急指针有效，表示报文段中有紧急数据，应尽快传送                |
| $ACK$ | 确认  | 确认号字段有效。只有 $ACK=1$ 时，确认号字段才有效           |
| $PSH$ | 推送  | 推送。接收 TCP 收到 $PSH=1$ 的报文段后，应尽快交付接收应用进程  |
| $RST$ | 复位  | 复位。$RST=1$ 表明 TCP 连接中出现严重差错，必须释放连接再重新建立 |
| $SYN$ | 同步  | 同步。用于连接建立                               |
| $FIN$ | 终止  | 终止。用于释放连接                               |

📌 **注意点**：  
当 $SYN=1, ACK=0$ 时，表示这是一个连接请求报文段；当 $SYN=1, ACK=1$ 时，表示这是一个连接接受报文段。最后$SYN=0, ACK=1$完成三次握手。

### 5.5.3 选项与填充 🧱

- **选项**：长度可变，最长可达 $40$ 字节。TCP 最初只规定了一种选项，即 <font color="orange">最大报文段长度 MSS</font>。
    
- **填充**：使整个 TCP 首部长度是 $4$ 字节的整数倍（与数据偏移匹配）。
...
$$\text{MTU (1500)} = \text{IP首部 (20)} + \text{TCP首部 (20)} + \text{MSS (1460)}$$
...
TCP 采用自适应算法，记录一个报文段**发出的时间以及收到相应确认的时间**，两者之差就是报文段的往返时间 $RTT$。

* **核心计算公式**

TCP 的 RTO 计算依赖于两个动态变量：平滑往返时间（加权平均往返时间值） $RTT_S$（Smoothed RTT）和 $RTT$ 的偏差平均值 $RTT_D$（RTT Deviation）。

1. 规则 A：当【第一次】测量到 RTT 样本值（$RTT_{new}$）时：

根据 RFC 标准，由于之前没有历史数据，初始化规则非常特殊：

 $RTT_S = RTT_{new}$
 $RTT_D = \frac{RTT_{new}}{2}$
 $RTO = RTT_S + 4 \times RTT_D$


2. 规则 B：当【之后持续】收到新的 RTT 样本值时：

使用加权移动平均公式更新（教材及 RFC 推荐参数通常取 $\alpha = \frac{1}{8} = 0.125$；$\beta = \frac{1}{4} = 0.25$）：

新的 $RTT_S = (1 - \alpha) \times \text{旧 } RTT_S + \alpha \times RTT_{new}$
新的 $RTT_D = (1 - \beta) \times \text{旧 } RTT_D + \beta \times |旧 RTT_S - RTT_{new}|$
$RTO = 新 RTT_S + 4 \times 新 RTT_D$
...
由于TCP是面向字节的，其发送时机就尤为重要。控制 TCP 发送报文段的时机，有三种机制：

1. **靠最大报文段长度（MSS）触发**：TCP 维持一个变量，它等于最大报文段长度 MSS。只要缓存中存放的数据达到 MSS 字节，就组装成一个 TCP 报文段发送出去。
    
2. **靠应用层显式推送**：由发送方应用进程指明要求发送报文段，即 TCP 支持的推送操作。
    
3. **靠特定定时器触发**：发送方的计时器期限到了，这时就把当前已有的缓存数据装入报文段，但长度不能超过 MSS。
...
**接收方产生糊涂窗口综合症的原因**：接收方应用进程消耗数据太慢，例如每次只读取 $1$ 字节，导致接收方不断通告很小的窗口。解决方法是让接收方等待一段时间，使接收缓存已有足够空间容纳一个最长的报文段，或者等到接收缓存已有一半空闲空间。只要出现两种情况之一，接收方再发出确认报文，并通知当前窗口大小。

⚠️ **易错点提醒**：糊涂窗口综合症既可能由**发送方**造成，也可能由**接收方**造成。发送方要避免发送很小的报文段，接收方也不要在缓存刚刚空出一点小空间时就急忙通知很小的窗口。
...
- **超时重传计时器超时**：表示网络已经出现拥塞。重新执行**慢开始**。
    
- **收到 3 个重复确认**：预示网络可能会出现拥塞。然后执行**快恢复**。
...
- **如果是由于【超时重传（Timeout）】发现的丢包**：说明网络彻底瘫痪，连 ACK 都回不来了。
    
    - **惩罚方案**：ssthresh 变成当前 cwnd 的一半；**cwnd 直接清零重来，跌回 1**。然后重新执行**慢开始**。
        
- **如果是由于【3 个重复 ACK（快重传）】发现的丢包**：说明网络只是轻微堵车。
    
    - **惩罚方案**：ssthresh 变成当前 cwnd 的一半；**cwnd 直接等于新的 ssthresh**。然后执行**快恢复**（直接开始线性增长）。
...
2. **第一次握手**：请求连接。A 的 TCP 向 B 主动发出连接请求报文段。首部中 $SYN=1$，并选择序号 $seq=x$，表示传送数据时第一个数据字节的序号是 $x$。
    
    $$\boxed{SYN=1,\quad seq=x}$$
    
1. **第二次握手**：确认并反向请求。B 收到连接请求后，如同意连接，就发回确认。确认报文段中 $SYN=1,\ ACK=1$，确认号 $ack=x+1$，并选择自己的序号 $seq=y$。
    
    $$\boxed{SYN=1,\ ACK=1,\ ack=x+1,\ seq=y}$$
    
1. **第三次握手**：最终确认。A 收到此报文段后向 B 给出确认，$ACK=1$，确认号 $ack=y+1$。若该 ACK 报文段不携带数据，则不消耗序号，下一个数据报文段的序号仍是 $seq=x+1$。
...
1. **第一次挥手**：主动关闭请求。A 的应用进程先向其 TCP 发出连接释放报文段，并停止再发送数据，主动关闭 TCP 连接。A 把连接释放报文段首部中的 $FIN=1$，其序号 $seq=u$，等待 B 的确认。
    
    $$\boxed{FIN=1,\quad seq=u}$$
    
1. **第二次挥手**：半关闭确认。B 发出确认，$ACK=1$，确认号 $ack=u+1$，该报文段序号为 $seq=v$。从 A 到 B 这个方向的连接就释放了，TCP 连接处于半关闭状态。B 若发送数据，A 仍要接收。
    
    $$\boxed{ACK=1,\quad ack=u+1,\quad seq=v}$$
    
1. **第三次挥手**：被动关闭请求。若 B 已经没有要向 A 发送的数据，其应用进程就通知 TCP 释放连接。B 发送 $FIN=1,\ ACK=1$，确认号仍为 $ack=u+1$。
    
    $$\boxed{FIN=1,\ ACK=1,\ ack=u+1}$$
    
1. **第四次挥手**：最终彻底断开。A 收到连接释放报文段后，必须发出确认。$ACK=1$，确认号 $ack=w+1$，自己的序号为 $seq=u+1$。
...
主动关闭方 A 发送最后一个 ACK 后，TCP 连接还没有立即释放，必须经过时间等待计时器设置的 $2MSL$ （两倍最长报文段寿命）后，A 才释放 TCP 连接。

必须等待 $2MSL$ 的原因有两个：

1. 保证 A 发送的最后一个 ACK 报文段能够到达 B。若这个 ACK 丢失，B 会重传 FIN，A 还能够再次发送 ACK。
    
2. 防止已失效的连接请求报文段（特指三次握手时的 **第一次握手**）出现在本连接中。
...
| **网络分层**  | **PDU 名称**         | **英文名**                     | **通俗理解**                         |
| --------- | ------------------ | --------------------------- | -------------------------------- |
| **应用层**   | 报文 / 消息            | Message                     | 应用程序真正想发的数据（如 HTTP 请求、AI API 调用） |
| **运输层**   | **TPDU** (报文段/数据报) | **TPDU** (Segment/Datagram) | 加了源端口、目的端口等运输层控制信息的包             |
| **网络层**   | IP 数据报 / 分组        | Packet / Datagram           | 加了源 IP、目的 IP 等路由信息的包             |
| **数据链路层** | 帧                  | Frame                       | 加了 MAC 地址、校验和等硬件链路信息的包           |
| **物理层**   | 比特流                | Bit                         | 变成光电信号在网线、空气中传输的 0 和 1           |

### 2. TCP协议中的计时器

1. 超时重传计时器 (Retransmission Timer) —— 应对“丢包”

这是 TCP 可靠传输机制的心脏，也是我们在前面讨论 ARQ 和拥塞控制时提到次数最多的计时器。

- **启动时机：** 每当发送方发出一个报文段时，就会为该报文段启动这个计时器。
    
- **超时动作：** 如果在计时器到期之前没有收到对应的确认（ACK），发送方就会认为该报文段已丢失，并**重新发送**该数据。
    
- **工程细节：** 它的超时时间（RTO，Retransmission TimeOut）不是固定的，而是根据网络往返时延（RTT）动态计算出来的。网络拥堵时 RTO 会拉长，网络顺畅时 RTO 会缩短。
    

2. 持续计时器 (Persistence Timer) —— 应对“零窗口死锁”

这个计时器专门服务于**流量控制**，用于防止发送方和接收方互相死等。

- **启动时机：** 当发送方收到接收方发来的“零窗口（rwnd = 0）”通知时，发送方必须停止发送数据，同时**立刻启动**持续计时器。
    
- **超时动作：** 计时器到期后，发送方会主动发送一个特殊的**零窗口探测报文段**（仅携带 1 字节数据）。
    
- **为什么要设计它？** 假设接收方腾出空间了，给发送方发了一个 `rwnd = 3000` 的更新报文，但**这个更新报文在半路上丢了**。此时，接收方在等发送方发数据，发送方在等接收方更新窗口，双方陷入死锁。持续计时器的探测机制就是为了打破这种僵局。
    

3. 时间等待计时器 (TIME-WAIT Timer / 2MSL 计时器) —— 应对“残影包”与“最后的一击”

这个计时器只在 TCP 的生命周期末尾（**四次挥手阶段**）闪亮登场。

- **启动时机：** 在四次挥手的最后一步，主动要求断开连接的一方（比如客户端）在发送出最后一个 ACK 报文后，会启动这个计时器，并进入 `TIME-WAIT` 状态。
    
- **超时动作：** 必须等待 **2MSL**（Maximum Segment Lifetime，报文段最大生存时间，通常是 2 分钟或 1 分钟）后，才真正彻底释放这个 Socket 端口和连接。
    
- **为什么要等这么久？** 
	1. **保证最后的 ACK 能送达：** 如果这最后一个 ACK 丢了，服务器会超时重发 `FIN` 报文。客户端留在这个状态，就是为了能再次回复 ACK。如果它立刻关机跑路，服务器就会收到 `RST`（复位报错）。
    
    2. **清理网络残影：** 确保本次连接中所有的“迷路数据包”都在网络中彻底自然消亡，绝对不会穿越到未来去污染下一个使用相同端口的新连接。
    

4. 保活计时器 (Keepalive Timer) —— 应对“对端猝死”

这个计时器主要用于清理那些已经没有任何数据的“僵尸连接”。

- **启动时机：** 只要服务器收到客户端的数据，就会重置这个计时器（通常默认设置长达 2 小时）。如果 2 小时内这段连接没有任何数据交互，计时器就会到期。
    
- **超时动作：** 服务器会向客户端发送一个探测报文。如果连续发送多个探测报文（通常是 10 个，每 75 秒发一次）后，客户端依然没有响应（可能已经拔网线、断电或崩溃），服务器就会认定客户端已经死亡，从而**强制终止连接**并释放宝贵的内存资源。
...
### 3. 网络协议首部梳理
![](ChatGPTImage2026年5月23日16_51_24.png)

| **协议层**                            | **协议单元**                   | **字段名称**                     | **占用大小**    | **核心作用与解析**                                                                                     |
| ---------------------------------- | -------------------------- | ---------------------------- | ----------- | ----------------------------------------------------------------------------------------------- |
| **运输层**<br><br>  <br>(Transport)   | **UDP 首部**<br>(共 8 字节)     | **源端口号 (Source Port)**       | 16 bit (2B) | 标识发送端进程（可选，不用时可全为 0）。                                                                           |
|                                    |                            | **目的端口号 (Destination Port)** | 16 bit (2B) | 标识接收端进程，把数据准确交付给对应应用。                                                                           |
|                                    |                            | **UDP 长度 (Length)**          | 16 bit (2B) | 包含 UDP 首部 + 数据的总字节数（最小为 8）。                                                                     |
|                                    |                            | **UDP 检验和 (Checksum)**       | 16 bit (2B) | 检验整个 UDP 报文段在传输中是否出错（包含伪首部）。                                                                    |
|                                    | **TCP 首部**<br>(固定 20 字节)   | **源/目的端口号**                  | 各 16 bit    | 作用同上，用于多路复用与多路分解。                                                                               |
|                                    |                            | **序号 (Sequence Number)**     | 32 bit (4B) | 本报文段所发送数据的**第一个字节的序号**，用于实现按序到达。                                                                |
|                                    |                            | **确认号 (Ack Number)**         | 32 bit (4B) | 期望收到对方下一个报文段的第一个数据字节的序号（**累计确认**）。                                                              |
|                                    |                            | **数据偏移 (Data Offset)**       | 4 bit       | 指出 TCP 报文段的数据起始处距离报文段起始处有多远（即**首部长度**）。                                                         |
|                                    |                            | **保留 (Reserved)**            | 6 bit       | 目前置 0，留待以后使用。                                                                                   |
|                                    |                            | **标志位 (Flags)**              | 6 bit       | 包含 **URG** (紧急)、**ACK** (确认有效)、**PSH** (推送)、**RST** (复位连接)、**SYN** (同步，建立连接)、**FIN** (终止，释放连接)。 |
|                                    |                            | **窗口大小 (Window)**            | 16 bit (2B) | 告知对方：目前自己的接收缓冲区还能容纳多少字节（用于**流量控制**）。                                                            |
|                                    |                            | **检验和 (Checksum)**           | 16 bit (2B) | 奇偶校验，范围包括首部、数据和 TCP 伪首部。                                                                        |
|                                    |                            | **紧急指针 (Urgent Pointer)**    | 16 bit (2B) | 当 URG = 1 时有效，指出本报文段中紧急数据的字节数。                                                                  |
| **网络层**<br><br>  <br><br>(Network) | **IPv4 首部**<br>(固定 20 字节)  | **版本 (Version)**             | 4 bit       | 通信使用的 IP 协议版本（IPv4 此时填 `4`，即 `0100`）。                                                           |
|                                    |                            | **首部长度 (IHL)**               | 4 bit       | IP 首部的长度。单位是 4 字节，固定 20 字节时该字段值为 `5` (`0101`)。                                                  |
|                                    |                            | **服务类型 (ToS)**               | 8 bit (1B)  | 区分服务类型，现多用于 QoS（服务质量）和拥塞通知。                                                                     |
|                                    |                            | **总长度 (Total Length)**       | 16 bit (2B) | 指 **IP 首部 + 数据**的总字节数。利用它可算出数据部分长度。                                                             |
|                                    |                            | **标识符 (Identification)**     | 16 bit (2B) | 计数器，同一原始 IP 数据报的所有分片都具有**相同的标识符**。                                                              |
|                                    |                            | **标志 (Flags)**               | 3 bit       | **DF** (Don't Fragment，不分片) 和 **MF** (More Fragments，还有后续分片)。                                   |
|                                    |                            | **片偏移 (Fragment Offset)**    | 13 bit      | 较长分组分片后，某分片在原分组中的**相对位置**（以 8 字节为单位）。                                                           |
|                                    |                            | **生存时间 (TTL)**               | 8 bit (1B)  | 数据报在网络中能经过的**最大路由器跳数**，每过一跳减 1，为 0 则丢弃，防止环路。                                                    |
|                                    |                            | **协议号 (Protocol)**           | 8 bit (1B)  | 指出此数据报携带的数据使用何种协议（如 **ICMP=1, TCP=6, UDP=17**）。                                                 |
|                                    |                            | **首部检验和 (Checksum)**         | 16 bit (2B) | **只校验数据报的首部**，不包括数据部分。                                                                          |
|                                    |                            | **源 IP 地址**                  | 32 bit (4B) | 发送端的 IP 地址。                                                                                     |
|                                    |                            | **目的 IP 地址**                 | 32 bit (4B) | 接收端的 IP 地址。                                                                                     |
| **数据链路层**<br><br>  <br>(Data Link) | **以太网帧首部**<br>(共 14 字节)    | **目的 MAC 地址**                | 48 bit (6B) | 接收端网卡的物理硬件地址（全球唯一）。                                                                             |
|                                    |                            | **源 MAC 地址**                 | 48 bit (6B) | 发送端网卡的物理硬件地址。                                                                                   |
|                                    |                            | **类型 (Type)**                | 16 bit (2B) | 标识上层（网络层）所使用的协议类型（如 **IPv4=0x0800, ARP=0x0806**）。                                               |
|                                    | **以太网帧尾部**<br><br>(共 4 字节) | **帧检验序列 (FCS)**              | 32 bit (4B) | 整个 MAC 帧的校验码，通常采用 **CRC-32 循环冗余校验**，用于滤除硬件传输错帧。                                                 |