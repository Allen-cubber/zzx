...
### 1. 组合逻辑环路
* `X <= Y` , `Y <= Z` , `Z <= X`**构建了一个没有时钟打断的信号反馈环路**。在数字逻辑设计中，**组合逻辑绝对不能有环路**。要打破这种死循环，需要把这个过程变成**时序逻辑**，让新值的计算基于“上一时刻”的旧值。

### 2. Case 语句覆盖不全
* 在 VHDL 中，`CASE` 语句必须是**穷举的**。意思是：选择信号 `sel` 的**每一个可能的取值**，都必须有对应的 `when` 语句来处理。
* ❌ 错误代码
```vhdl
case sel is
    when "00" => Y <= A;
...
### 3. IF 语句覆盖不全
* 在组合逻辑中，如果 `IF` 语句**没有写 `ELSE`**，或者没有涵盖所有条件，综合器会认为：“当条件不满足时，保持原来的值”，生成锁存器 。
* ❌ 错误代码
```vhdl
process(sel, A)
begin
...
begin
    temp <= A + B;      -- 第一步：加法
    Y    <= temp * C;   -- 第二步：乘法 (错误发生在这里！)
end process;
```
* ✅ 正确写法：**把信号移出 Process** 利用并发语句的特性。
```vhdl
-- 并发执行，连线会自动处理依赖关系
temp <= A + B;
Y    <= temp * C;
